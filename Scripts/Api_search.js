...
mediaSelect.addEventListener('change', () => {
    let key = searchInput.value;
    let media = mediaSelect.value === "Movies" ? "movie" : "tv";
    SearchFunction(key, media);
...
function SearchFunction(key, media = "movie", GR = '') {
    ...
    let searchType = media === "movie" ? "movie" : "tv";
    fetch(`https://api.themoviedb.org/3/search/${searchType}?query=${key}&page=${page}&api_key=${api_key}&with_genres=${GR}`)
        ...
        .then(res => {
            ...
            results.forEach(Movie => {
                ...
                MovieCard.href = `./preview.html?id=${id}&page=${page}&URL=search/${searchType}?&GR=${genre_ids[0]}&Type=${searchType}`;
                // MovieCard.href = `./preview.html?id=${id}&page=${page}&URL=discover/${searchType}?language=en-US&GR=${GR}&Type=Movie`;
                ...
                <div class="MovieNum">
                    <span class="Movie-Type">${searchType === "movie" ? "Movie" : "TV Series"}</span>
