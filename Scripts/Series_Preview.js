    ...
    .then(response => response.json())
     .then(res => {
        let Movies = res.results;
            // console.log(`-------------- Related Series -------`);
        Movies.forEach(Movie => {
            // console.log(`Series id is : ${Movie.id}`);
            if(Movie_id == Movie.id)
            {
                ...
                <h5 id="serieOriginalName">${name==original_name?'':original_name}</h2>
...
// function that handles the overly
    ...
    .then(res =>
        {
            let Serie = res;
            console.log(Serie);
                let {created_by,episode_run_time,homepage,in_production,languages,last_air_date,last_episode_to_air,next_episode_to_air,networks,number_of_episodes,number_of_seasons,origin_country,popularity,production_companies,production_countries} = Serie;
                languages.forEach(lng =>{document.getElementById('MoviesP-lng').innerHTML = lng;});
                let PreviewPopularity = document.getElementById('MoviesP-Popularity');
                PreviewPopularity.innerHTML += popularity;
                let Data2 = document.querySelector('.MovieData-2');
                Data2.innerHTML =
                `
                       <div class="py-10" style="line-height: 24px;font-style: italic;">
                            <p class="episode_run_time">Episode run time : ${episode_run_time?episode_run_time:''} min</p>
                            <p> Streaming right now on : <a href="${homepage}">click here !</a></p>
                            <p>in production? : ${in_production?'Yes':'No'}</p>
                            <ol class="production_countries"></ol>
                        </div>
                `; 
                created_by.forEach(Directer =>
                    {
                      let {id,name,gender,profile_path} = Directer;
                      console.log(Directer);
                      profile_path?document.querySelector('.CreaterCover').src = img_url+profile_path:RegularPoster;
                      name?document.querySelector('.CreaterName').innerHTML = name:'Movies_4u';
                    })
        })
}
fetchMoreData();
