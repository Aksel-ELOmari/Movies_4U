                        ...
                        genres.forEach(g =>
                            {
                                ...
                                if(G.id === g)
                                {
                                    // console.log(`thos two are mathes : ${G.id} ${g} so the Gerne of the Movie is : ${G.name}`);
...
const SerieVideos_api = `https://api.themoviedb.org/3/tv/${Movie_id}?api_key=${api_key}&append_to_response=videos`;

// Videos_api?fetchVideos(Videos_api):fetchVideos(SerieVideos_api);
TYPE == `Movie`?fetchVideos(Videos_api):fetchVideos(SerieVideos_api);
function fetchVideos(api){
    ...
    .then(res => {
            ...
            {
                alert('ther is no videos for this Movie Sorry!!!')
